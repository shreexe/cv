import cv2
import mediapipe as mp#multimodal framework w/ x,y,z coordinates
import numpy as np
import math
import random
import time

user_response = None #will have yes/no
button_pressed = False #yes/no pressed 

BUTTON_WIDTH = 100
BUTTON_HEIGHT = 50
BUTTON_MARGIN = 20

def get_button_positions(frame_width, frame_height):#button position based on height and width of the frame
    ...
    return (yes_button_top_left, yes_button_bottom_right,
            no_button_top_left, no_button_bottom_right)

def mouse_callback(event, x, y, flags, param):#checks whether yes or no was clicked 
...
def calculate_ear(landmarks, eye_indices):#Eye aspect ratio
...
def calculate_mar(landmarks, mouth_indices):#Mouth Aspect ratio
    A = math.dist(landmarks[mouth_indices[0]], landmarks[mouth_indices[4]])
    B = math.dist(landmarks[mouth_indices[1]], landmarks[mouth_indices[5]])  
    C = math.dist(landmarks[mouth_indices[2]], landmarks[mouth_indices[6]])  
    D = math.dist(landmarks[mouth_indices[3]], landmarks[mouth_indices[7]])#width  
    mar = (A + B + C) / (2.0 * D)
    return mar
...
def main():
    global user_response, button_pressed
    cap = cv2.VideoCapture(0)#video frame input 
    if not cap.isOpened():
    ...
    while True:
        ...
        frame_height, frame_width = frame.shape[:2] #gets the frame height and width
        yes_tl, yes_br, no_tl, no_br = get_button_positions(frame_width, frame_height)

        if waiting_for_response:
            #UI for KYC Prompt
            cv2.putText(frame, "Are you ready for KYC?", (BUTTON_MARGIN, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

            cv2.rectangle(frame, yes_tl, yes_br, (0, 255, 0), -1)
            cv2.putText(frame, "YES", (yes_tl[0] + 20, yes_tl[1] + 35),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)

            cv2.rectangle(frame, no_tl, no_br, (0, 0, 255), -1)
            cv2.putText(frame, "NO", (no_tl[0] + 25, no_tl[1] + 35),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

            cv2.setMouseCallback('KYC Prompt', mouse_callback,
                                 param=(yes_tl, yes_br, no_tl, no_br))
            ...
            if button_pressed:
                if user_response == 'yes':
                    ...
                    print("User chose YES. Starting KYC process.")
            ...
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)#BGR to RGB for mediapipe
            results = face_mesh.process(rgb_frame)#mediapipe processes the rgb frame

            if results.multi_face_landmarks:
                face_landmarks = results.multi_face_landmarks[0]
                landmarks = [(int(point.x * frame_width), int(point.y * frame_height)) for point in face_landmarks.landmark]#Calculating the landmarks acc to the H and W pixelfashion
    ...
    if liveness_confirmed:
        print("Liveness confirmed! Proceeding with KYC process...")
    elif user_response == 'no':
        print("KYC process was not initiated by the user.")
    else:
        print("Liveness not confirmed or actions not detected, maybe a spoof..!")

if __name__ == "__main__":
    cv2.namedWindow('KYC Prompt')
    main()
